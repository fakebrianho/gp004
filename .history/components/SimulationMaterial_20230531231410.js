...
const SimulationMaterial = shaderMaterial(
	...
	void main() {
        ...
        // vec2 finalOriginal = mix(original, original1, uProgress);
        // float originalDistance = length(finalOriginal);
        // vec2 rotatedOriginal = rotationMatrix(uTime) * finalOriginal;
        // float rotatedDistance = length(rotatedOriginal);
        ...
        // velocity *= 0.95;
        // vec2 velocity = texture2D( uCurrentPosition, vUv ).zw;
        vec2 rotatedVelocity = rotationMatrix(uTime) * velocity;
        ...
        rotatedVelocity *= 0.95;
    
        // particle attraction to shape force
        // vec2 direction = normalize( finalOriginal - position );
        // vec2 direction = normalize( finalOriginal - rotatedPosition );
        // float dist = length( finalOriginal - position );
        // float dist = length( rotatedOriginal - position );
        // if( dist > 0.01 ) {
        //     velocity += direction  * 0.0001;
        // }
        vec2 direction = normalize( rotatedFinalOriginal - rotatedPosition );
        float dist = length( rotatedFinalOriginal - rotatedPosition );
        if( dist > 0.01 ) {
            rotatedVelocity += direction  * 0.0001;
        }
        ...
        rotatedPosition.xy += rotatedVelocity;
    
        
        gl_FragColor = vec4( rotatedPosition, rotatedVelocity);
	}
	`
)
