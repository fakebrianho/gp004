...
export default function Particles(props) {
	...
	useFrame(({ gl, mouse }, delta) => {
		...
		if (simMat.current) {
			...
			mouseDebug.current.position.x = mousePos.x
			mouseDebug.current.position.y = mousePos.y
			// console.log(mouseDebug.current.position, mousePos)
			// console.log(mousePos)
			const mp = {
			...
			simMat.current.uniforms.uMouse.value.x = mp2.x
			simMat.current.uniforms.uMouse.value.y = mp2.y
			simMat.current.uniforms.uTime.value =
				(simMat.current.uniforms.uTime.value + delta) % (2.0 * Math.PI)

			// renderMat.current.rotation.value += 0.01
			// console.log(renderMat.current)
			// materialRef.current.uniforms.uTime.value = (materialRef.current.uniforms.uTime.value + delta) % (2.0 * Math.PI);
			particleRef.current.rotation.z += 0.01
			// console.log(particleRef.current.rotation)
