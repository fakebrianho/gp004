...
async function getPixelDataFromImage(url, SIZE, NUMBER) {
	...
	for (let i = 0; i < SIZE; i++) {
		for (let j = 0; j < SIZE; j++) {
			...
			let rng = pixels[Math.floor(Math.random() * pixels.length)]
			data[4 * index] = rng.x
			data[4 * index + 1] = rng.y
			data[4 * index + 2] = 0
			data[4 * index + 3] = 0
		}
	}
...
export default function Particles() {
	...
	let tempTexture
	Promise.all([getPixelDataFromImage('/tester.png', SIZE, NUMBER)]).then(
		(texture) => {
			tempTexture = texture
		}
	)
